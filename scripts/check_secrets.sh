...
set -eu
# Screenshots live only under docs/screens/; a PNG anywhere else is a mistake.
...
if git diff --cached --name-only | grep -E '\.png$' | grep -Ev '^docs/screens/' ; then
  echo "check_secrets: refusing — screenshots belong under docs/screens/" >&2
  exit 1
fi
